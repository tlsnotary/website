...
*Medians over 10 samples per row. Both modes use the realistic ZK authentication path on sent and recv. See [Benchmark setup](#benchmark-setup).*
...
As disucssed in the [Proxy mode introcution post](https://tlsnotary.org/blog/2026/04/22/proxy-mode) there are a few practical considerations beyond raw runtime:
...
- **TCP_NODELAY:** enabled.
- **Samples:** 10 per row; charts show medians with min/max bands.
