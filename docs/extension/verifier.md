...
The verifier server additionally accepts sessions in [proxy mode](/docs/protocol/proxy-mode) on the same deployment.
...
- **(Proxy mode)** — alternatively, forwards TLS traffic via a session-bound WebSocket proxy and verifies a post-session ZK proof; see the [proxy mode reference](/docs/protocol/proxy-mode)
...
:::caution Naming note
This WebSocket proxy is a **transport bridge** and is distinct from [**proxy mode**](/docs/protocol/proxy-mode). In proxy mode the same `/proxy` endpoint additionally accepts a `sessionId` query parameter that binds the forwarded TLS traffic to a verification session.
:::
